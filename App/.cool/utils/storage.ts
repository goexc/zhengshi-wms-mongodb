...
class Storage {
	 ...
	 *
	 * @param key 存储键名
	 * @returns 存储的数据；键不存在时 uni.getStorageSync 返回空字符串，调用方需检查类型
	 *
	 * @example
	...
	isExpired(key: string): boolean {
		// getStorageSync 返回 any；Android 旧缓存可能把数字时间戳还原为字符串。
		// 先按运行时类型收窄，避免 `as number` 在 JVM 上触发 ClassCastException。
		const rawValue = uni.getStorageSync(`${key}${EXPIRES_SUFFIX}`);
		let expireTime = 0;

		if (typeof rawValue == "number") {
			expireTime = rawValue;
		} else if (typeof rawValue == "string" && rawValue != "") {
			expireTime = parseFloat(rawValue);
		} else {
			return true;
		}

		// 缺失、空值或损坏的过期时间都按已过期处理。
	 ...
	 *
	 * @param key 存储键名
	 * @returns 存储的数据；键不存在时返回空字符串，与 get 保持一致
	 *
	 * @example
