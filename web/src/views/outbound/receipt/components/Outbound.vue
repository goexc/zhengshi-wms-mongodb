...
import {
  CustomerOutbondReceiptTypes,
  OutboundReceiptMaterialStatus,
  OutboundReceiptMaterialStatusText, SupplierOutbondReceiptTypes
} from "@/enums/outbound.ts";
import {OutboundReceiptMaterialRequest} from "@/api/outbound/types.ts";
import {reqUpdateOutboundReceiptMaterial} from "@/api/outbound";
...
let submit = async () => {
  ...
  let req:OutboundReceiptMaterialRequest = {
    id: receipt.value.id,
  ...
  materials.value.forEach(item=>{
    req.materials.push({
      id: item.id,
      status: OutboundReceiptMaterialStatusText[item.status]??'',
      actual_quantity: item.actual_quantity,
    })
  })

  let res =await reqUpdateOutboundReceiptMaterial(req)
   if(res.code === 200){
     ElMessage.success(res.msg)
     emit('success')
   }else{
...
<template>
  <el-form
    ...
    <el-form-item v-if="SupplierOutbondReceiptTypes.includes(receipt.type)" label="供应商">
      {{receipt.supplier_name}}
    </el-form-item>
    <el-form-item v-if="CustomerOutbondReceiptTypes.includes(receipt.type)" label="客户">
  ...
  <el-table
    ...
    <el-table-column label="物料规格" prop="model" width="200"/>
    <el-table-column label="计划数量" prop="estimated_quantity" align="center"/>
    <el-table-column label="实际数量" prop="actual_quantity" align="center" width="200">
    ...
    <el-table-column label="出库状态" prop="status" align="center">
      <template #default="{row, col, $index}">
        <el-select
          ...
          placeholder="请选择出库状态"
        >
          <el-option v-for="(item, idx) in OutboundReceiptMaterialStatus.filter(one=>one.value>=form.materials[$index].status)" :key="idx" :label="item.label" :value="item.value"/>
        </el-select>
      </template>
    </el-table-column>
  </el-table>
