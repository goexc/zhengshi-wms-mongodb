...
enum API {
  ...
  //获取货位分页接口
  BINS_URL = "/warehouse_bin",
  //添加货位
  ADD_BIN_URL = "/warehouse_bin",
  //修改货位
  UPDATE_BIN_URL = "/warehouse_bin",
  //修改货位状态
...
export const reqWarehouseBins = (req: WarehouseBinsRequest) =>
  request.get<any, WarehouseBinsResponse>(API.BINS_URL, { params: req });

//添加或修改货位的接口方法
export const reqAddOrUpdateWarehouseBin = (data: WarehouseBinRequest) => {
  if (data.id.trim().length === 0) {
    //添加
    return request.post<any, baseResponse>(API.ADD_BIN_URL, data);
  } else {
    //修改
    return request.put<any, baseResponse>(API.UPDATE_BIN_URL, data);
  }
};
