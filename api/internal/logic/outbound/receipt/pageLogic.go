...
func (l *PageLogic) Page(req *types.OutboundReceiptsRequest) (resp *types.OutbondReceiptsResponse, err error) {
	resp = new(types.OutbondReceiptsResponse)

	//1.筛选
	var filter = bson.M{}
	if strings.TrimSpace(req.Code) != "" {
		fmt.Println("查询出库单编号：", req.Code)
		filter["code"] = strings.TrimSpace(req.Code)
	}

	if strings.TrimSpace(req.Status) != "" {
		filter["status"] = code.OutboundReceiptStatusCode(req.Status)
	}
	...
	cur, err := l.svcCtx.OutboundReceiptModel.Find(l.ctx, filter, opt)
	if err != nil {
	...
	var receipts = make([]model.OutboundReceipt, 0)
	...
	var list = make([]types.OutboundReceiptItem, 0)
	for _, one := range receipts {
		var receipt = types.OutboundReceiptItem{
			Id:            one.Id.Hex(),
			Code:          one.Code,
			Status:        code.OutboundReceiptStatusText(one.Status),
			Type:          one.Type,
		...
		for _, material := range one.Materials {
			receipt.Materials = append(receipt.Materials, types.OutboundMaterialItem{
	...
	resp.Data.Total, err = l.svcCtx.OutboundReceiptModel.CountDocuments(l.ctx, filter)
	if err != nil {
