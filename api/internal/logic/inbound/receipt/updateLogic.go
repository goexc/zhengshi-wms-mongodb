...
func (l *UpdateLogic) Update(req *types.InboundReceiptUpdateRequest) (resp *types.BaseResponse, err error) {
	...
	//if _, ok := canEditStatus[code.InboundReceiptStatusText(receipt.Status)]; !ok {
	if _, ok := canEditStatus[receipt.Status]; !ok {
		resp.Code = http.StatusBadRequest
	...
	//“审核不通过”时，修改为“待审核”
	//if code.OutboundOrderStatusCode("审核不通过") == receipt.Status {
	if "审核不通过" == receipt.Status {
		//receipt.Status = code.OutboundOrderStatusCode("待审核")
		receipt.Status = "待审核"
	}
	...
	_, err = l.svcCtx.InboundReceiptModel.UpdateByID(l.ctx, receiptId, &update)
	if err != nil {
